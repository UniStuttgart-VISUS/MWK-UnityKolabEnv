...
namespace Photon.Voice.IOS
{
    public enum AudioSessionCategory // values are the same as in AudioIn.mm enums
    {
        ...
        Ambient = 0, // // API_AVAILABLE(ios(3.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

        /*  Use this category for background sounds.  Other music will stop playing. */
        SoloAmbient = 1, // API_AVAILABLE(ios(3.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

        /* Use this category for music tracks.*/
        Playback = 2, // API_AVAILABLE(ios(3.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

        /*  Use this category when recording audio. */
        Record = 3, // API_AVAILABLE(ios(3.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

        /*  Use this category when recording and playing back audio. */
        PlayAndRecord = 4, // API_AVAILABLE(ios(3.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

        /*  Use this category when using a hardware codec or signal processor while
         not playing or recording audio. */
        AudioProcessing = 5, // API_DEPRECATED("No longer supported", ios(3.0, 10.0)) API_UNAVAILABLE(watchos, tvos) API_UNAVAILABLE(macos);
        ...
        /*  Use this category to customize the usage of available audio accessories and built-in audio hardware.
         ...
         outputs connected.  */
        MultiRoute = 6, // API_AVAILABLE(ios(6.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);
    }
    ...
    public enum AudioSessionMode // values are the same as in AudioIn.mm enums
    {
        ...
        /* The default mode */
        Default = 0, // API_AVAILABLE(ios(5.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

        /* Only valid with AVAudioSessionCategoryPlayAndRecord.  Appropriate for Voice over IP
        (VoIP) applications.  Reduces the number of allowable audio routes to be only those
        that are appropriate for VoIP applications and may engage appropriate system-supplied
        signal processing.  Has the side effect of setting AVAudioSessionCategoryOptionAllowBluetooth */
        VoiceChat = 1, // API_AVAILABLE(ios(5.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);
...
//        GameChat = 2, // API_AVAILABLE(ios(5.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);
        ...
        VideoRecording = 3, // API_AVAILABLE(ios(5.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);
        ...
        Measurement = 4, // API_AVAILABLE(ios(5.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);
        ...
        MoviePlayback = 5, // API_AVAILABLE(ios(6.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

        /* Only valid with kAudioSessionCategory_PlayAndRecord. Reduces the number of allowable audio
        routes to be only those that are appropriate for video chat applications. May engage appropriate
        system-supplied signal processing.  Has the side effect of setting
        AVAudioSessionCategoryOptionAllowBluetooth and AVAudioSessionCategoryOptionDefaultToSpeaker. */
        VideoChat = 6, // API_AVAILABLE(ios(7.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);
...
//        VoicePrompt = 8, // API_AVAILABLE(ios(12.0), watchos(5.0), tvos(12.0)) API_UNAVAILABLE(macos);
    ...
    public enum AudioSessionCategoryOption // values as deined in Apple Audio Session API
    {
        /*
