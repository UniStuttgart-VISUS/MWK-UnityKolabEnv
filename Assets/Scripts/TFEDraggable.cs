...
using GrabberPool = HTC.UnityPlugin.Utility.ObjectPool<TFEDraggable.Grabber>;

// demonstrate of dragging things useing built in EventSystem handlers
public class TFEDraggable : GrabbableBase<TFEDraggable.Grabber>
...
{
    [Serializable]
    public class UnityEventDraggable : UnityEvent<TFEDraggable> { }
    ...
    public void OnGrabTransform()
    {
        ...
        var localTargetPos = transform.InverseTransformPoint(targetPose.pos);
        localTargetPos.x = transform.localPosition.x;
        localTargetPos.z = transform.localPosition.z;
        localTargetPos.y = Mathf.Clamp(localTargetPos.y, -3.0f, 0.0f);
