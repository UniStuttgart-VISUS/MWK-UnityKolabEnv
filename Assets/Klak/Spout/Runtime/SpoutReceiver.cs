...
namespace Klak.Spout
{
    ...
    public sealed class SpoutReceiver : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            {
                if (ptr != _sharedTexture.GetNativeTexturePtr() && !ptrs.Contains(ptr))
                {
                    ptrs.Add(ptr);
                    if (ptrs.Count >= 2)
                    {
                        // Not match: Destroy to get refreshed.
                        Debug.Log("ptr change, clear all: "+ptr);
                        Util.Destroy(_sharedTexture);
                        ptrs.Clear();
                    }                  
                }
                if (width != _sharedTexture.width ||
                    height != _sharedTexture.height)
                {
                    // Not match: Destroy to get refreshed.
                    Debug.Log("size change, clear all: "+ptr);
                    Util.Destroy(_sharedTexture);
                }
            }
